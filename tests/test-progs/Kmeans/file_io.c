                   ...
                   float **objects)       /* [numObjs][numCoords] */
{
    ...
    if (isBinaryFile) {  /* using MPI-IO to read file concurrently */
        ...
        int size = read(infile, &i, sizeof(int));
        ...
        /* read the objects */
        size = read(infile, objects[0], numObjs * numCoords * sizeof(float));
        if ( size != numObjs * numCoords * sizeof(float)){
            printf("could not read file\n");
            close(infile);
            exit(-1);
        }
