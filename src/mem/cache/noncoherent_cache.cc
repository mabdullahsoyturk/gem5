...
{
    // As this a non-coherent cache located below the point of coherency, we do not expect requests that are used to keep caches coherent (e.g., InvalidateReq or UpdateReq).
    assert(pkt->isRead() || pkt->isWrite());
    BaseCache::satisfyRequest(pkt, blk);
}

bool NoncoherentCache::access(PacketPtr pkt, CacheBlk *&blk, Cycles &lat, PacketList &writebacks)
{
...
void NoncoherentCache::handleTimingReqMiss(PacketPtr pkt, CacheBlk *blk, Tick forward_time, Tick request_time)
{
    // miss
...
PacketPtr NoncoherentCache::createMissPacket(PacketPtr cpu_pkt, CacheBlk *blk,
                                   bool needs_writable,
...
Cycles NoncoherentCache::handleAtomicReqMiss(PacketPtr pkt, CacheBlk *&blk,
...
void NoncoherentCache::serviceMSHRTargets(MSHR *mshr, const PacketPtr pkt, CacheBlk *blk)
{
...
PacketPtr NoncoherentCache::evictBlock(CacheBlk *blk)
{
...
NoncoherentCache* NoncoherentCacheParams::create()
{
    assert(tags);
