...
BaseCache::recoverIntermittentFaults()
{
    ...
    for(std::vector<CacheFault>::iterator it = faults.begin(); it != faults.end(); ++it) {
        if(faultInjector->isIntermittent(*it) && it->cacheToBeInserted == cacheType && it->recovered == 0) {
            DPRINTF(FaultTrace, "Intermittent found %#x %d\n", it->blockAddr, it->recovered);
            it->recovered = 1;
...
{
    if (writeAllocator &&pkt && pkt->isWrite() && !pkt->req->isUncacheable()) {
        writeAllocator->updateMode(pkt->getAddr(), pkt->getSize(), pkt->getBlockAddr(blkSize));
    }
...
BaseCache::recvTimingReq(PacketPtr pkt)
{
    ...
    DPRINTF(FlowTrace, "recvTimingReq started\n");
    ...
    DPRINTF(FlowTrace, "recvTimingReq ended\n");
}
...
{
    DPRINTF(FlowTrace, "recvTimingResp started\n");
    ...
    DPRINTF(FlowTrace, "recvTimingResp ended\n");
    delete pkt;
}
...
{
    DPRINTF(FlowTrace, "satisfyRequest started\n");
    assert(pkt->isRequest());
    ...
    if (pkt->cmd == MemCmd::SwapReq) {
        ...
        faultInjector->injectFaults(pkt, blkSize, true, cacheType);
    ...
    DPRINTF(FlowTrace, "satisfyRequest ended\n");
}
...
BaseCache::access(PacketPtr pkt, CacheBlk *&blk, Cycles &lat,
                  PacketList &writebacks)
{
    DPRINTF(FlowTrace, "access started\n");
    // sanity check
    assert(pkt->isRequest());
    ...
    if (pkt->isWriteback()) {
        ...
        DPRINTF(FlowTrace, "satisfyRequest worked, access returns true for %#x\n", pkt->getAddr());
        DPRINTF(FlowTrace, "access ended\n");
    ...
    DPRINTF(FlowTrace, "satifyRequest did not work, access returns false for %#x\n", pkt->getAddr());
    DPRINTF(FlowTrace, "access ended\n");
...
CacheBlk*
BaseCache::handleFill(PacketPtr pkt, CacheBlk *blk, PacketList &writebacks, bool allocate)
{
    DPRINTF(FlowTrace, "handleFill started\n");
    assert(pkt->isResponse());
    ...
    if (pkt->isRead()) {
        ...
        DPRINTF(FlowTrace, "handlefill isRead worked for %#x\n", pkt->getAddr());

        pkt->writeDataToBlock(blk->data, blkSize);
    }
    DPRINTF(FlowTrace, "handlefill isRead did not work for %#x\n", pkt->getAddr());
    // The block will be ready when the payload arrives and the fill is done
    blk->setWhenReady(clockEdge(fillLatency) + pkt->headerDelay + pkt->payloadDelay);

    DPRINTF(FlowTrace, "handleFill finished\n");

    return blk;
}

CacheBlk* BaseCache::allocateBlock(const PacketPtr pkt, PacketList &writebacks)
{
    // Get address
...
BaseCache::sendMSHRQueuePacket(MSHR* mshr)
{
    ...
    if (!memSidePort.sendTimingReq(pkt)) {
        ...
        // schedSendEvent (we will wait for a ret3ry before
